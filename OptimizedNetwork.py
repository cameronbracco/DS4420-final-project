...
import torch
from torch import argmax, zeros, full, where, logical_or, nonzero, randperm, sum, cat, amax, mean
...
class BetterSONN:
    ...
    def __init__(
        ...
        positive_quantilizer_step: Tuple[int, int] = (19, 1),
        negative_quantilizer_step: Tuple[int, int] = (150, 1),
        ...
        self.positive_quantilizer_step = positive_quantilizer_step
        self.negative_quantilizer_step = negative_quantilizer_step
        ...
        self.connection_masks = zeros(
            (self.output_size, self.num_neurons_per_column, self.input_size),  # Shape
            dtype=torch.uint8,
            device=device
        )
        ...
        self.positive_quantilizers = Quantilizer(output_size, *self.positive_quantilizer_step, device=device)
        self.negative_quantilizers = Quantilizer(output_size, *self.negative_quantilizer_step, device=device)
    ...
    def learn(self, x, y_true):
        ...
        avg_spike_count = int(mean(spike_counts.float()))

        # Isolate just the false columns and find the maximum spike count
        false_column_spikes = cat((spike_counts[0:y_true], spike_counts[y_true + 1:]))
        max_false_spike = amax(false_column_spikes)

        # UPDATE QUANTILIZERS - POSITIVE
        delta_true = spike_counts[y_true] - max_false_spike
        if self.positive_quantilizers.check_pos(delta_true, y_true):
            positive_reinforcements.append(y_true)
            self.weight_arrays[y_true] += self.positive_reinforce_amount

        delta_false = spike_counts - avg_spike_count
        reinforce_mask = self.negative_quantilizers.check_negs(delta_false, y_true)
        negative_reinforcements.extend(list(reinforce_mask.nonzero()))
        self.weight_arrays[reinforce_mask] -= self.negative_reinforce_amount
    ...
    def forward(self, x):
        ...
        # ------ Select prediction ------
        prediction_idx = argmax(col_spike_counts).item()
    ...
    def count_spikes(self, x):
        ...
        all_neuron_potentials = sum(x * self.connection_masks * self.weight_arrays, dim=2)

        # Whether a neuron is spiking. Has shape [columns x neurons per column]
        all_neuron_spikes = where(all_neuron_potentials < self.spike_threshold, 0, 1)

        # Calculate total spikes per column. Has shape [columns]
        col_spike_counts = sum(all_neuron_spikes, dim=1)
    ...
    def grow(self):
        ...
        curr_connection_counts = sum(self.connection_masks, dim=2)
        ...
        neurons_to_update = nonzero(curr_connection_counts != self.num_connections_per_neuron)
        connections_grown = 0

        for neuron_idx in neurons_to_update:  # tensorize?
            # Identify all the open connections for this neuron (open is when it's 0)
            open_connections = nonzero(self.connection_masks[neuron_idx[0], neuron_idx[1]] == 0)
            rand_indices = randperm(open_connections.shape[0])
            ...
            self.connection_masks[neuron_idx[0], neuron_idx[1], rand_indices] = 1
            self.weight_arrays[neuron_idx[0], neuron_idx[1], rand_indices] = self.initial_connection_weight
    ...
    def prune(self):
        ...
        self.connection_masks = where(
            logical_or(
                self.weight_arrays < self.prune_weight,
